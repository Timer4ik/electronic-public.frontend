...
export const cartSlice = createSlice({
    ...
    reducers: {
        ...
        clearCartItems:(state, action: PayloadAction<number[]>) => {
            state.cartItems = state.cartItems?.filter(item => 
                !action.payload.find(i => item.id == i))
            setJsonCart(state.cartItems || [])
        },
        clearCart: (state) => {
            state.cartItems = []
            setJsonCart([])
...
export const { addItemToCart, initCart, clearCart,clearCartItems, decrementCartItemAmount, deleteItemFromCart, incrementCartItemAmount } = cartSlice.actions
